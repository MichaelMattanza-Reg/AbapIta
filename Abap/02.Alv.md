...
```abap
  DATA:lv_fieldname TYPE string,
       fs_desc TYPE REF TO cl_abap_structdescr,
  ...
  CREATE DATA dref TYPE ('ty_outtab').
  ASSIGN dref->* TO <fs_dref>.
   fs_desc ?=  cl_abap_structdescr=>describe_by_data( <fs_dref> ) .

  LOOP AT fs_desc->components INTO wa_fields.
    CONCATENATE '<fs_dref>' '-' wa_fields-name INTO lv_fieldname .
    ASSIGN (lv_fieldname) TO <fs_fname> .
    ls_fcat-col_pos = sy-index.
    lr_typedescr = cl_abap_typedescr=>describe_by_data( <fs_fname> ) .

    ls_fcat-ref_field = lr_typedescr->absolute_name+6.
    ls_fcat-fieldname = wa_fields-name.
    ls_fcat-outputlen = lr_typedescr->length.
  ...
  ENDLOOP.
    ...
    SELECT rollname, scrtext_m
      ...
      WHERE rollname EQ @lt_fcat-ref_field
      AND ddlanguage EQ @sy-langu.

  LOOP AT lt_fcat ASSIGNING FIELD-SYMBOL(<fs_fcat>).
    IF line_exists( lt_coldescr[ rollname = <fs_fcat>-ref_field ] ).
      <fs_fcat>-scrtext_m = lt_coldescr[ rollname = <fs_fcat>-ref_field ]-scrtext_m.
    ENDIF.
  ENDLOOP.
...
```abap
class ZREG_ALV_MANAGER definition
  public
  create public .
...
public section.
  ...
  types:
    tty_outtab TYPE TABLE OF zqm_val_forn .

  data ALV type ref to CL_GUI_ALV_GRID .
  data GT_OUTTAB type TTY_OUTTAB .
...
CLASS ZREG_ALV_MANAGER IMPLEMENTATION.


* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZREG_ALV_MANAGER->HANDLE_TOOLBAR
* +-------------------------------------------------------------------------------------------------+
* | [--->] E_OBJECT                       LIKE
* | [--->] E_INTERACTIVE                  LIKE
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZREG_ALV_MANAGER->HANDLE_USER_COMMAND
* +-------------------------------------------------------------------------------------------------+
* | [--->] E_UCOMM                        LIKE
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD handle_user_command.
    ...
    DATA: lt_rows     TYPE lvc_t_row,
          lt_sel_rows TYPE TABLE OF se16n_seltab.
    ...
    CASE e_ucomm.
      WHEN 'HISTORICAL'.
        ...
        IF lines( lt_rows ) EQ 1.
          historycal( i_row = lt_rows[ 1 ]-index ).
        ENDIF.
